...
async def _run_python(script: Path, env: Optional[Dict[str, str]] = None, args: Optional[List[str]] = None, cwd: Optional[Path] = None) -> Dict[str, Any]:
  ...
  if args:
    cmd.extend(args)
  log_file = (cwd or script.parent) / "service_runtime.log"
...
async def _ensure_repo(service: ServiceConfig) -> None:
  ...
  if proc.returncode != 0:
    raise RuntimeError(f"Failed to clone {service.repo_url}: {stderr.decode()}")
...
async def update_env_and_restart(
  ...
  """
  write_result = materialize_env_file(service_key, values, allowed_keys=allowed_keys, backup=True)
