...
class BrainDriveRAGCommunityLifecycleManager(CommunityPluginLifecycleBase):
    ...
    self.required_services_runtime = [
      {
        ...
        "healthcheck_url": "http://localhost:18000/health",
      ...
      },
      {
        ...
        "install_command": "python3.11 service_scripts/install_with_venv.py --full",
        "start_command": "python3.11 service_scripts/start_with_venv.py",
        "healthcheck_url": "http://localhost:18080/health",
        "definition_id": SETTINGS_DEFINITION_ID,
        "required_env_vars": []
      }
    ]
  ...
  async def _perform_user_installation(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
    try:
      ...
      health = await self._collect_service_health()
  ...
  async def _perform_user_update(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
    ...
    return {"success": True, "service_installs": installs, "required_env_vars": env_map}
  ...
  def _maybe_encrypt(self, table: str, field: str, value: str) -> str:
  ...
  async def _ensure_settings(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
    try:
      ...
      default_settings_value = {
        ...
        "document_chat_mode": "venv",
        "document_chat_port": 18000,
        "document_processing_enabled": True,
        "document_processing_mode": "venv",
        "document_processing_port": 18080,
        "full_install": False
      }
      ...
      if not existing_instance:
        instance_data = {
          ...
          "scope": "user",
          "user_id": user_id,
          "value": json.dumps(default_settings_value),
          "created_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
          "updated_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
      ...
      await db.commit()
      return {"success": True, "definition_id": definition_id, "instance_id": instance_id}
    except Exception as error:  # pragma: no cover
      await db.rollback()
  ...
  async def _prepare_services(self, user_id: str) -> Dict[str, Any]:
    ...
    async_mode = os.environ.get("RAG_ASYNC_INSTALL", "1").lower() in {"1", "true", "yes", "on"}
    use_jobs = os.environ.get("RAG_USE_JOB_MANAGER", "1").lower() in {"1", "true", "yes", "on"}
    ...
    if async_mode:
      ...
      for key in service_keys:
        loop.create_task(prepare_service(key, full_install=full_install, force_recreate=force_recreate))
        installs.append({"service": key, "scheduled": True})
      return {"skipped": False, "installs": installs, "mode": "async"}
    else:
      for key in service_keys:
      ...
      return {"skipped": False, "installs": installs, "mode": "sync"}
  ...
  async def materialize_env_without_restart(self, env_payload: Dict[str, Dict[str, str]]) -> Dict[str, Any]:
    ...
    for service_key, values in env_payload.items():
      allowed = env_map.get(service_key, [])
      write_result = materialize_env_file(service_key, values, allowed_keys=allowed, backup=True)
